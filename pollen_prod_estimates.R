...
### calculate pollen production per tree, in parallel to propagate uncertainty in pollen production equations, classification ################
library(furrr)
library(future)
    for(k in 1:1){ #including uncertainty from classification
    
    #focal_genus_id <- "Quercus" #focal_genus_id <- genus_list[k]
    focal_genus_id <- genera_with_equations[k]
    
    
    # ---- Wrap terra objects so they survive being passed to worker processes ----
    raster_template_wrapped <- wrap(raster_template)
    nyc_boundary2_wrapped   <- wrap(nyc_boundary2)  # only if this is a SpatVector/SpatRaster;
    # skip wrap/unwrap below if it's e.g. an sf object
    
    # ---- Turn the j-loop body into a function ----
    process_chunk <- function(j, focal_genus_id, tr, id_lookup, all_cells, dist_result,
                              raster_template_wrapped, nyc_boundary2_wrapped) {
      
      library(terra)
      library(tidyverse)
      library(arrow)
      
      # unwrap terra objects inside the worker
      raster_template <- rast(raster_template_wrapped)
      nyc_boundary2    <- vect(nyc_boundary2_wrapped)  # adjust if not a SpatVector
      ...
      for (i in 1:10) {
        ...
        Qupa_param_b <- rnorm(n = 1, mean = 8.30, sd = 6.88)
        Ulsp_param_a <- rnorm(n = 1, mean = 1.263, sd = 0.420)
        Ulsp_param_b <- rnorm(n = 1, mean = -38.904, sd = 98.023)
        ...
        it_dbh_genus_np_i <-
          ...
          mutate(per_tree_pollen_prod = case_when(
            Genus_it == "Acer" & Species == "Acer negundo" ~ (Acne_param_a * tree_area_c + Acne_param_b) * 0.558,
            Genus_it == "Acer" & Species == "Acer platanoides" ~ Acpl_param_a * tree_area_c + Acpl_param_b,
            Genus_it == "Acer" & Species == "Acer rubrum" ~ (Acru_param_a * tree_area_c + Acru_param_b) * 0.106,
            Genus_it == "Acer" & Species == "Acer saccharinum" ~ Acsa_param_a * tree_area_c + Acsa_param_b,
            Genus_it == "Acer" & is.na(Species) ~ 0.6311 * (Acpl_param_a * tree_area_c + Acpl_param_b) +
              0.1713 * ((Acru_param_a * tree_area_c + Acru_param_b) * 0.106) +
              0.1668 * (Acsa_param_a * tree_area_c + Acsa_param_b) +
              0.0011 * ((Acne_param_a * tree_area_c + Acne_param_b) * 0.558),
            Genus_it == "Betula" ~ Bepa_param_a * tree_area_c + Bepa_param_b,
            Genus_it == "Gleditsia" ~ Gltr_param_a * tree_area_c + Gltr_param_b,
            Genus_it == "Juglans" ~ Juni_param_a * tree_area_c + Juni_param_b,
            Genus_it == "Morus" ~ ((exp(Mosp_param_a * tree_area_c + Mosp_param_b)) / 1000000000) * 0.578,
            Genus_it == "Platanus" ~ Plac_param_a * tree_area_c + Plac_param_b,
            Genus_it == "Populus" ~ (Posp_param_a * tree_area_c + Posp_param_b) * 0.482,
            Genus_it == "Quercus" & !is.na(Species) ~ Qusp_param_a * tree_area_c + Qusp_param_b,
            Genus_it == "Quercus" & Species == "Quercus palustris" ~ Qupa_param_a * tree_area_c + Qupa_param_b,
            Genus_it == "Quercus" & is.na(Species) ~ 0.6546 * (Qusp_param_a * tree_area_c + Qusp_param_b) +
              0.3454 * (Qupa_param_a * tree_area_c + Qupa_param_b),
            Genus_it == "Ulmus" ~ (Ulsp_param_a * tree_area_c + Ulsp_param_b),
            .default = 0
          )) %>%
          ...
          mutate(per_tree_pollen_prod = case_when(per_tree_pollen_prod < 0 ~ 0,
                                                  per_tree_pollen_prod > 0 ~ per_tree_pollen_prod)) %>%
          mutate(iter = i, chunk_j = j, focal_genus = focal_genus_id) %>%
          select(Poly_ID, Genus_Ref, Species, tree_area_c, species_it, per_tree_pollen_prod, iter, chunk_j, focal_genus)
        ...
        cell_sum_prod_i <- it_dbh_genus_np_i %>%
          inner_join(id_lookup, by = "Poly_ID") %>%
          group_by(grid_cell) %>%
          summarize(sum_pol = sum(per_tree_pollen_prod, na.rm = TRUE),
                    n = n(), had_obs = TRUE, .groups = "drop") %>%
          complete(grid_cell = all_cells$grid_cell,
                   fill = list(sum_pol = 0, n = 0, had_obs = FALSE)) %>%
          filter(!is.na(grid_cell))
        
        r_sum <- raster_template
        values(r_sum) <- NA
        r_sum[cell_sum_prod_i$grid_cell] <- cell_sum_prod_i$sum_pol
        
        cell_nums <- cells(r_sum, nyc_boundary2)[, "cell"]
        na_in_poly <- cell_nums[is.na(r_sum[cell_nums][, 1])]
        r_sum[na_in_poly] <- 0
        
        r_sum_raster_list_chunk_j[[i]] <- wrap(r_sum)
        
        focal_genus_dist <- filter(dist_result, focal_genus == focal_genus_id) %>% pull(param_dist)
        focal_matrix <- focalMat(r_sum, d = focal_genus_dist, type = "circle", fillNA = TRUE)
        focal_matrix_no_weights <- focal_matrix
        ...
        prod_dist_focal_r_sum <- focal(r_sum, w = focal_matrix_no_weights, fun = "sum", na.rm = TRUE)
        names(prod_dist_focal_r_sum) <- "mean_prod_ha_within_dist"
        focal_window_area <- (focal_genus_dist^2 * pi) / 10000
        prod_dist_focal_rate_ha <- prod_dist_focal_r_sum / focal_window_area
        
        prod_dist_raster_list_chunk_j[[i]] <- wrap(prod_dist_focal_rate_ha)
      }
      
      saveRDS(r_sum_raster_list_chunk_j,
              paste0("C:/Users/dsk273/Desktop/prod_per_ha_raster_list/", "prod_raster_list_", focal_genus_id, "_", j, ".rds"))
      saveRDS(prod_dist_raster_list_chunk_j,
              paste0("C:/Users/dsk273/Desktop/prod_within_dist_raster_list/", "prod_dist_raster_list_", focal_genus_id, "_", j, ".rds"))
      write_parquet(x = it_dbh_genus_np_all,
                    sink = paste0("C:/Users/dsk273/Desktop/prod_chunk/indiv_tree_pol_pred_", focal_genus_id, "_", j, ".parquet"))
      
      paste(focal_genus_id, ": chunk", j, "completed at", Sys.time())
    }
    
    # ---- Run in parallel across the 100 chunks ----
    
    plan(multisession, workers = 4) #parallel::detectCores() - 2)
    
    chunk_messages <- future_map(
      1:100,
      process_chunk,
      focal_genus_id = focal_genus_id,
      tr = tr,
      id_lookup = id_lookup,
      all_cells = all_cells,
      dist_result = dist_result,
      raster_template_wrapped = raster_template_wrapped,
      nyc_boundary2_wrapped = nyc_boundary2_wrapped,
      .progress = TRUE,
      .options = furrr_options(seed = TRUE)   # ensures proper random number streams across workers
    )
    
    plan(sequential)
    
    walk(chunk_messages, print)
    ...
    r_stats_dist <- app(r_stack_dist, fun = function(x) c(mean = mean(x, na.rm = TRUE),
                                                          sd   = sd(x, na.rm = TRUE)))
    names(r_stats_dist) <- c("prod_dist_mean", "prod_dist_sd") #plot(r_stats)
...
#         
#         #focal_genus_id <- "Quercus"
#         focal_genus_id <- genera_with_equations[j]
#         
...
#         for (i in seq_along(files_to_read_genus)) {
...
#         #summary_paths <- dir("C:/Users/dsk273/Desktop/prod_chunk_pixel_tibble/intermediate_summaries/", full.names = TRUE)
#         summary_paths <- dir("C:/Users/dsk273/Desktop/prod_chunk_pixel_tibble/intermediate_summaries/", full.names = TRUE) %>% 
#           stringr::str_subset(., focal_genus_id)
#         
#         final_summary <- summary_paths %>% #summary_paths[1:2] %>% 
#           map(read_rds) %>%
#           list_rbind() %>%
#           group_by(grid_cell) %>%
#           summarize(
...
#         
#         
#         #create a raster of total pollen production per ha (mean of each iteration)
...
#         #fill in NAs with zeros
#             #get the cell numbers of the raster that fall within the nyc boundary
#             cell_nums <- cells(r_mean, nyc_boundary2)[, "cell"]
#             
#             # among those cells, find which ones are NA
#             na_in_poly <- cell_nums[is.na(r_mean[cell_nums][,1])]
#             
#             # replace those NA cells with 0
#             r_mean[na_in_poly] <- 0
#             #plot(r_mean) #check that NA values were filled in
#         
#         
#         
#         #create a raster of the sd of total pollen production (sd among iterations)
...
#           #fill in NAs with zeros
#             #get the cell numbers of the raster that fall within the nyc boundary
#             cell_nums <- cells(r_sd, nyc_boundary2)[, "cell"]
#             
#             # among those cells, find which ones are NA
#             na_in_poly <- cell_nums[is.na(r_sd[cell_nums][,1])]
#             
#             # replace those NA cells with 0
#             r_sd[na_in_poly] <- 0
#             #plot(r_sd) #check that NA values were filled in
#         
#         
#         r_n <- raster_template
#         values(r_n) <- NA
...
#                                      focal_genus_id, "_1ha_summary_raster_v2.tif")
...
# 
#     
# ### calculate sum pollen production and SD within selected distance for each genus #####################################
#   
...
#     #focal_genus_id <- "Quercus" #Morus Acer Gleditsia Platanus
#     
#     #load in focal genus 1 ha raster
#     genus_raster_name <- paste0( "C:/Users/dsk273/Box/classes/plants and public health fall 2025/class project analysis/July26_reanalysis/",
#                                  focal_genus_id, "_1ha_summary_raster.tif")
#     prod_raster <- rast(genus_raster_name, lyrs = 1)
#     #plot(prod_raster)
#     
#   
#     
...
#     focal_genus_dist <- filter(dist_result, focal_genus == focal_genus_id) %>% 
#                         pull(param_dist)
#     focal_matrix <- focalMat(prod_raster, d = focal_genus_dist, type = "circle", fillNA = TRUE)
#     focal_matrix_no_weights <- focal_matrix
#     focal_matrix_no_weights[focal_matrix_no_weights > 0] <- 1    # Replace all values > 0 with 1 to create an unweighted window
#     
#     #calculate pollen production within focal_distance
#     prod_dist_focal_sum <-  focal( prod_raster, w = focal_matrix_no_weights, fun = "sum", na.rm = TRUE)
#     names(prod_dist_focal_sum) <- "mean_prod_within_dist"
#     focal_window_area <- (focal_genus_dist^2 * pi)/10000 #divide by 10000 to convert to ha
#     prod_dist_focal_rate_ha <- prod_dist_focal_sum/focal_window_area 
#     
#     plot(prod_dist_focal_rate_ha)
#     
#     writeRaster(prod_dist_focal_rate_ha, 
#                 paste0("C:/Users/dsk273/Box/classes/plants and public health fall 2025/class project analysis/July26_reanalysis/",
#                        "mean_production_within_",focal_genus_dist, "m_", focal_genus_id, ".tif"), overwrite = TRUE)
#     
#     # #a more detailed map
#     # ggplot() + ggthemes::theme_few() + ggtitle(focal_genus) + 
#     #   #geom_spatraster_rgb(data = nyc_topo_spatrast) +
#     #   geom_spatraster(data = prod_dist_focal_sum/1000, alpha = 0.6) +
#     #   scale_fill_viridis_c(na.value = "transparent", 
#     #                        #option = "plasma",
#     #                        name = "pollen produced within dist \n (trillions of grains)",
...
#           stringr::str_subset(., focal_genus_id)
#         
#         final_summary <- summary_paths %>% #summary_paths[1] %>% 
#           map(read_rds) %>%
#           list_rbind() %>%
#           group_by(grid_cell) %>%
#           summarize(
